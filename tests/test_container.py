...
"""Tests per il decoratore @container.
...
- naming esplicito: @container('nome')
...
def test_container_marks_function():
    @container
    def block(self, pane, x):
        pane.div(value=x)

    assert hasattr(block, "_decorator")
    assert block._decorator.get("container") is True


def test_container_requires_body():
    with pytest.raises(ValueError, match="must have a body"):
        @container
        def empty(self, pane): ...


def test_container_preserves_signature():
    @container
...
def test_container_importable_top_level():
    from genro_builders import container as sm
    assert sm is container


# --- Sotto-step 3: dispatch ---------------------------------------------------

def test_container_invoked_via_node():
    class P(HtmlBuilder):
        @container
        def block(self, pane):
            pane.h1("ok")
...
def test_container_receives_builder_as_self_and_node_as_pane():
    received = {}
...
def test_container_args_kwargs_forwarded():
    class P(HtmlBuilder):
        @container
...
def test_container_explicit_name():
    """@container('alias') sovrascrive il naming automatico."""
...
def test_container_collision_different_funcs_raises():
...
def test_container_subclass_override_same_name_is_ok():
    """Una subclass che ridichiara @container con stesso func.__name__
...
def test_container_dispatched_via_active_builder():
    """A container is resolved through the builder of the node it is
...
def test_container_return_value_passed_through():
    class P(HtmlBuilder):
        @container
