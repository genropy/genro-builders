...
Usage:
    ...
    python -m genro_builders.contrib.html.importer.html5_schema_builder \\
        path/to/rng/ -o html5_schema.bag.mp

    # From local RNC directory (auto-converts to RNG)
    python -m genro_builders.contrib.html.importer.html5_schema_builder \\
        path/to/rnc/ -o html5_schema.bag.mp
    ...
    python -m genro_builders.contrib.html.importer.html5_schema_builder \\
        path/to/rng/ -o html5_schema.bag.mp --json -v

This tool is a **manual diff utility**, not a codegen pipeline. The
output is meant to be compared against the hand-maintained
``html5_elements.py`` mixin to spot W3C drift. See
``importer/README.md`` for the comparison workflow.
...
def convert_rnc_to_rng(rnc_dir: Path, rng_dir: Path) -> list[str]:
    ...
    """
    # rnc2rng has no --help / --version flag; running it with empty stdin
    # produces an empty grammar (returncode 0). Use that as a presence check.
    try:
        subprocess.run(
            ["rnc2rng"], input=b"", capture_output=True, check=True
        )
...
def build_schema(rng_dir: Path, verbose: bool = False) -> Bag:
    ...
    The output bag is the **intermediate artifact** consumed by the
    codegen step that produces ``html5_elements.py``. Each top-level
    node carries the bare element label (e.g. ``div``, ``br``) and a
    ``sub_tags`` attribute with the comma-separated allowed children.

    The codegen step reads this bag (via ``.bag.json`` / ``.bag.mp``)
    and emits one ``@element(sub_tags=...)`` per node. The runtime
    consumer (``HtmlBuilder``) never sees this bag — it sees the
    generated Python mixin.
    ...
    Returns:
        Schema Bag ready for serialization via ``to_tytx``.
    """
    schema = Bag()
    ...
    for e in sorted(elements):
        ...
        schema.set_item(e, None, sub_tags=sub_tags)
