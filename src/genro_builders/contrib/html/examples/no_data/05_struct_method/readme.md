...
A `@struct_method` is a reusable block invoked **from a node**, not
...
falls back to the struct method registered on the builder, called with
`pane=body`.
...
- pass an explicit name with `@struct_method("alias")`.
