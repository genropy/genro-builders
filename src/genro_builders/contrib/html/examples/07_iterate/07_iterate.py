...
class CatalogBuilder(HtmlBuilder):
    ...
    def product_card(self, comp, **kwargs):
        ...
        ^.?name reads the 'name' attribute from the current data node.
        Each iteration resolves ^.? against its own node's attributes.
        """
...
class ProductCatalog(BuilderManager):
    ...
    def store(self, data):
        # Data as Bag nodes with attributes — iterate reads these
        products = Bag()
        products.set_item(
            "p0", None,
    ...
    def main(self, source):
        ...
        body.p("Each card is generated by iterate — one per product in the data Bag.")

        catalog = body.div(_class="catalog")

        # iterate='^products': replicate product_card once per child node
